...
const formatCurrency = (amount: number, currency: string) => {
  ...
  useEffect(() => {
    const fetchInvoice = async () => {
      try {
        setLoading(true)
        const response = await axios.get(`${import.meta.env.VITE_BASE_URL}api/getInvoice/${companyId}/${"debit"}`)
  ...
  <div className="relative max-h-[90vh] w-full max-w-3xl overflow-y-auto rounded-lg bg-white shadow-xl p-6">
    ...
    {/* Form Component */}
    <PurchaseOrderForm invoiceType="debit" />
  </div>
</div>
 )
      }
      {!viewInvoice ? (
        ...
        <div className="relative">
          ...
          <div className="flex items-center justify-between p-4 border-b border-gray-200 bg-gray-50 dark:bg-gray-800">
            <h2 className="text-lg font-semibold">Debit Note Preview</h2>
            <div className="flex items-center gap-2">
              <button
                onClick={downloadPDF}
              ...
              >
                ...
                <div className="flex justify-between items-start mb-8">
                  ...
                  <div className="text-right">
                    <h1 className="text-4xl font-bold text-gray-800 mb-4">DEBIT NOTE</h1>
                    <div className="space-y-2 text-gray-700">
                      <div>
                        <span className="font-semibold">Debit Note #:</span> {viewInvoiceData.invoiceNumber}
                      </div>
                      <div>
